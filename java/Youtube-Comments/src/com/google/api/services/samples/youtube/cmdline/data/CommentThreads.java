    ...
    public static void main(String[] args) {
         ...
         */
         ...
         /*   CommentThreadListResponse videoCommentsListResponse = youtube.commentThreads()
                    .list("snippet").setVideoId(videoId).setMaxResults((long) 100).setTextFormat("plainText").execute();
             String pagetoken =videoCommentsListResponse.getNextPageToken();
             
            System.out.println("  - next page token: " + pagetoken);
            List<CommentThread> videoComments = videoCommentsListResponse.getItems();
            */
            Boolean flag = true;
             long comments = 0;
             String pagetoken ="";
            while(flag==true && comments >=0 )
            {
            	CommentThreadListResponse videoCommentsListResponse = youtube.commentThreads()
                        .list("snippet,replies").setVideoId(videoId).setMaxResults((long) 100).setPageToken(pagetoken).setTextFormat("plainText").execute();
                 pagetoken =videoCommentsListResponse.getNextPageToken();
                 
                System.out.println("  - next page token: " + pagetoken);
                List<CommentThread> videoComments = videoCommentsListResponse.getItems();
            	//comments = comments + videoComments.size();
            ...
            if (videoComments.isEmpty()) {
                ...
                for (CommentThread videoComment : videoComments) {
                 CommentSnippet snippet = videoComment.getSnippet().getTopLevelComment()
                            .getSnippet();
                CommentThreadReplies findkind = videoComment.getReplies();
                if(findkind !=null)
                {
                	List<Comment> commentReply = findkind.getComments();
                	 for(Comment reply: commentReply)
                	 {
                		 CommentSnippet replysnippet = reply.getSnippet();
                		 System.out
                         .println("\n================== Comment Video Replies ==================\n");
                             System.out.println("  - Author: " + replysnippet.getAuthorDisplayName());
                             System.out.println("  - Comment: " + replysnippet.getTextDisplay());
                             System.out.println("  - PublishedAt: " + replysnippet.getPublishedAt());
                             System.out.println("  - UpdatedAt: " + replysnippet.getUpdatedAt());
                ...
                .println("\n================== Returned Video Comments ==================\n");
                    ...
                    System.out.println("  - UpdatedAt: " + snippet.getUpdatedAt());
